...
from data_apis.corpus import NLPCCCorpus
from data_apis.data_utils import NLPCCDataLoader
# from models.cvae import KgRnnCVAE
from models.escvae import EsCVAE

# constants
tf.app.flags.DEFINE_string("word2vec_path", './data/sgns.weibo.word', "The path to word2vec. Can be None.")
tf.app.flags.DEFINE_string("data_dir", "data/nlpcc.json", "Raw data directory.")
...
def main():
    ...
    # get data set
    api = NLPCCCorpus(FLAGS.data_dir, word2vec=FLAGS.word2vec_path, word2vec_dim=config.embed_size)
    dial_corpus = api.get_corpus()
    ...
    train_feed = NLPCCDataLoader("Train", train_dial, config)
    valid_feed = NLPCCDataLoader("Valid", valid_dial, config)
    test_feed = NLPCCDataLoader("Test", test_dial, config)
    ...
    with tf.Session(config=gpu_config) as sess:
        ...
        with tf.variable_scope(scope, reuse=None, initializer=initializer):
            model = EsCVAE(sess, config, api, log_dir=None if FLAGS.forward_only else log_dir, forward=False, scope=scope)
        with tf.variable_scope(scope, reuse=True, initializer=initializer):
            valid_model = EsCVAE(sess, valid_config, api, log_dir=None, forward=False, scope=scope)
        with tf.variable_scope(scope, reuse=True, initializer=initializer):
            test_model = EsCVAE(sess, test_config, api, log_dir=None, forward=True, scope=scope)
