...
from models.cvae import BaseTFModel

class EsCVAE(BaseTFModel):
    ...
    def __init__(self, sess, config, api, log_dir, forward, scope=None):
        ...
        with tf.name_scope("io"):
            self.input_contexts = tf.placeholder(dtype=tf.int32, shape=(None, self.max_utt_len), name="input_contexts")
            # self.floors = tf.placeholder(dtype=tf.int32, shape=(None, None), name="floor")
            self.input_lens = tf.placeholder(dtype=tf.int32, shape=(None,), name="input_lens")
            self.input_emotions = tf.placeholder(dtype=tf.int32, shape=(None,), name="input_emotions")
            ...
            self.output_emotions = tf.placeholder(dtype=tf.int32, shape=(None,), name="output_emotions")
        ...
        with variable_scope.variable_scope("emotionEmbedding"):
            t_embedding = tf.get_variable("embedding", [self.emotion_vocab_size, config.topic_embed_size], dtype=tf.float32)
            inp_emotion_embedding = embedding_ops.embedding_lookup(t_embedding, self.input_emotions)
            outp_emotion_embedding = embedding_ops.embedding_lookup(t_embedding, self.output_emotions)
        ...
        with variable_scope.variable_scope("wordEmbedding"):
            ...
            if config.sent_type == "rnn":
                ...
                output_embedding, _ = get_bi_rnn_encode(output_embedding, sent_cell, self.output_lens, scope="sent_rnn")

            elif config.sent_type == "bi_rnn":
                fwd_enc_cell = self.get_rnncell(config.cell_type, self.context_cell_size, keep_prob=1.0, num_layer=config.num_layer)
                bwd_enc_cell = self.get_rnncell(config.cell_type, self.context_cell_size, keep_prob=1.0, num_layer=config.num_layer)
                _, enc_last_state = tf.nn.bidirectional_dynamic_rnn(
                    fwd_enc_cell, bwd_enc_cell, input_embedding, dtype=tf.float32, sequence_length=self.input_lens)
        ...
        with variable_scope.variable_scope("contextRNN"):
            ...
            attribute_fc1 = layers.fully_connected(outp_emotion_embedding, 30, activation_fn=tf.tanh, scope="attribute_fc1")

            cond_embedding = tf.concat([inp_emotion_embedding, enc_last_state],1)
        ...
        with variable_scope.variable_scope("generationNetwork"):
            ...
            self.da_logits = layers.fully_connected(meta_fc1, self.emotion_vocab_size, scope="da_project")
        ...
        with variable_scope.variable_scope("decoder"):
            ...
            if forward:
                loop_func = decoder_fn_lib.context_decoder_fn_inference(None,
                                                                        dec_init_state,
                                                                        embedding,
        ...
        if not forward:
            with variable_scope.variable_scope("loss"):
                ...
                da_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.da_logits, labels=self.output_emotions)
    ...
    def batch_2_feed(self, batch, global_t, use_prior, repeat=1):
        ...
        inputs, input_lens, input_emotions, outputs, output_lens, output_emotions = batch
        feed_dict = {self.input_contexts: inputs, self.input_lens: input_lens,
                     self.input_emotions: input_emotions, self.output_tokens: outputs,
                     self.output_lens: output_lens, self.output_emotions: output_emotions,
    ...
    def test(self, sess, test_feed, num_batch=None, repeat=5, dest=sys.stdout):
        ...
        while True:
            ...
            true_src_lens = feed_dict[self.input_lens]
            true_outs = feed_dict[self.output_tokens]
            true_topics = feed_dict[self.input_emotions]
            true_das = feed_dict[self.output_emotions]
            local_t += 1
            ...
            for b_id in range(test_feed.batch_size):
                ...
                dest.write("Batch %d index %d of topic %s\n" % (local_t, b_id, self.emotion_vocab[true_topics[b_id]]))
                ...
                da_str = self.emotion_vocab[true_das[b_id]]
                ...
                for r_id in range(repeat):
                    ...
                    dest.write("Sample %d (%s) >> %s\n" % (r_id, self.emotion_vocab[pred_da], pred_str))
