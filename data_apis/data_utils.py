...
class NLPCCDataLoader(LongDataLoader):
    ...
    def epoch_init(self, batch_size, backward_size, step_size, shuffle=True, intra_shuffle=True):
        ...
        self.batch_indexes = []
        for i in range(temp_num_batch):
            tmp_batch_indexes = self.indexes[i * self.batch_size:(i+1) * self.batch_size]
            if intra_shuffle:
        ...
        # TODO
        self.grid_indexes = self.batch_indexes


    def _prepare_batch(self, cur_grid, prev_grid=None):

        sentence_dat = [self.data[idx] for idx in cur_grid]

        input_utts, input_lens, input_emos, output_utts, output_lens, output_emos = [], [], [], [], [], []

        for (post, post_e), (response, response_e) in sentence_dat:
            input_utts.append(self.pad_to(post))
            input_lens.append(len(post))
            input_emos.append(post_e)

            output_utts.append(self.pad_to(response, do_pad=False))
            output_lens.append(len(response))
            output_emos.append(response_e)

        vec_inputs = np.zeros((self.batch_size, self.max_utt_size), dtype=np.int64)
        vec_input_lens = np.array(input_lens, dtype=np.int64)
        vec_input_emos = np.array(input_emos, dtype=np.int64)

        vec_outputs = np.zeros((self.batch_size, np.max(output_lens)), dtype=np.int64)
        vec_output_lens = np.array(output_lens, dtype=np.int64)
        vec_output_emos = np.array(output_emos, dtype=np.int64)

        for idx in range(self.batch_size):
            vec_outputs[idx, 0:vec_output_lens[idx]] = output_utts[idx]
            vec_inputs[idx] = input_utts[idx]

        return vec_inputs, vec_input_lens, vec_input_emos, vec_outputs, vec_output_lens, vec_output_emos
